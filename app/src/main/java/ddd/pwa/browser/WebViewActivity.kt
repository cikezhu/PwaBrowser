...
class WebViewActivity : AppCompatActivity() {
    ...
    override fun onResume() {
        super.onResume()
        // 在页面恢复时触发自动填充
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && ::autofillManager.isInitialized) {
    ...
    private fun initializeWebView() {
        ...
        settings.saveFormData = true // ✅ 启用表单数据保存（重要！）
...
//        settings.cacheMode = WebSettings.LOAD_CACHE_ELSE_NETWORK
    ...
    private fun configureAutofillSupport() {
        // 为 Android 8.0+ 启用自动填充
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            // 关键配置：启用 WebView 自动填充
            myWebView.importantForAutofill = View.IMPORTANT_FOR_AUTOFILL_YES
            
            // 设置自动填充提示，帮助系统识别表单类型
            myWebView.setAutofillHints(
            ...
            Log.d(mTAG, "自动填充支持已启用")
        } else {
            Log.d(mTAG, "自动填充需要 Android 8.0+，当前版本: ${Build.VERSION.SDK_INT}")
        }
    }
...
private class WVViewClient(private val _context: Context, private val _m: WebViewActivity):
    ...
    override fun onPageFinished(view: WebView?, url: String?) {
        ...
        // 触发自动填充（在页面加载完成后）
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && _m::autofillManager.isInitialized) {
            _m.autofillManager.requestAutofill(_m.myWebView)
        }
